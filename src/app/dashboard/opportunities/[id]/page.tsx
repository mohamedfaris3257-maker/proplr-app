...
export default async function OpportunityDetailPage({ params }: OpportunityDetailPageProps) {
  ...
  const [{ data: portfolioItems }, { data: application }, { data: savedItem }] = await Promise.all([
    ...
    supabase.from('applications').select('status').eq('user_id', user.id).eq('opportunity_id', params.id).maybeSingle(),
    supabase.from('saved_items').select('id').eq('user_id', user.id).eq('item_id', params.id).eq('item_type', 'opportunity').maybeSingle(),
  ]);

  const applicationStatus: ApplicationStatus | null = (application?.status as ApplicationStatus) ?? null;

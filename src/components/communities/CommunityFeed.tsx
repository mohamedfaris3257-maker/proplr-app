...
export function CommunityFeed({
  ...
  const [communities, setCommunities] = useState<CommunityOption[]>(myCommunities);
  ...
  async function handleJoinCommunity(communityId: string) {
    try {
      ...
      if (data.status === 'approved') {
        // Immediately add to communities list
        const joined = discoverCommunities.find((c: any) => c.id === communityId);
        if (joined) {
          const newCommunity: CommunityOption = { id: joined.id, name: joined.name, type: joined.type };
          setCommunities(prev => [...prev, newCommunity]);
          if (!selectedCommunity) setSelectedCommunity(joined.id);
        }
        setJoinStatuses(prev => ({ ...prev, [communityId]: 'approved' }));
        // Refresh feed to include new community posts
    ...
    <div style={{ flex: 1, overflowY: 'auto', background: '#f0f2f8', minHeight: '100vh' }}>
      <div style={{ display: 'flex', gap: 24, maxWidth: 1128, margin: '0 auto', padding: '24px 16px', alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: '0 0 240px', position: 'sticky', top: 20 }}>
          ...
          {discoverCommunities.length > 0 && (
            <div style={{ background: '#fff', borderRadius: 12, border: '0.5px solid rgba(0,0,0,0.1)', padding: '12px 16px' }}>
              ...
              {discoverCommunities.filter((c: any) => !communities.some(mc => mc.id === c.id)).slice(0, 4).map((c: any) => (
                <div
                  key={c.id}
                  style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 0', borderBottom: '0.5px solid rgba(0,0,0,0.06)' }}
                >
                  ...
                  {joinStatuses[c.id] === 'pending' ? (
                    <span style={{ fontSize: 10, fontWeight: 700, color: '#f59e0b', background: 'rgba(245,158,11,0.1)', padding: '3px 10px', borderRadius: 100 }}>Pending</span>
                  ) : joinStatuses[c.id] === 'approved' ? (
                    <span style={{ fontSize: 10, fontWeight: 700, color: '#22c55e', background: 'rgba(34,197,94,0.1)', padding: '3px 10px', borderRadius: 100 }}>Joined</span>
                  ) : (
                    <button
                      onClick={() => handleJoinCommunity(c.id)}
                      style={{ fontSize: 11, fontWeight: 700, color: '#0ea5e9', background: 'rgba(14,165,233,0.08)', border: '1px solid rgba(14,165,233,0.2)', borderRadius: 100, padding: '4px 12px', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' as any }}
        ...
        <div style={{ flex: 1, maxWidth: 600, minWidth: 0 }}>
          ...
          <div style={{ background: '#fff', borderRadius: 12, border: '0.5px solid rgba(0,0,0,0.1)', padding: '12px 16px', marginBottom: 8 }}>
            <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 10 }}>
              ...
              <button
                ...
                style={{ flex: 1, textAlign: 'left', background: 'transparent', border: '1px solid rgba(0,0,0,0.15)', borderRadius: 32, padding: '10px 16px', fontSize: 14, color: '#666', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 600 }}
                onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = '#f3f2ef'; }}
                onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = 'transparent'; }}
              >
                Start a post
          ...
          {/* Community tabs — pill switcher */}
          <div style={{ background: '#fff', borderRadius: 12, border: '0.5px solid rgba(0,0,0,0.1)', padding: '12px 16px', marginBottom: 8 }}>
            {/* Current community indicator */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <div style={{ width: 8, height: 8, borderRadius: '50%', background: activeCommunity === 'all' ? '#f59e0b' : '#3d9be9' }} />
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                <button style={{ fontSize: 12, fontWeight: 700, color: '#000', background: 'none', border: 'none', cursor: 'pointer', fontFamily: 'inherit' }}>Top ▾</button>
              </div>
            </div>
            {/* Pill tabs */}
            <div style={{ display: 'flex', gap: 6, overflowX: 'auto', scrollbarWidth: 'none' as any, padding: 3, background: '#f0f2f8', borderRadius: 100 }}>
              <button onClick={() => setActiveCommunity('all')} style={{
                padding: '8px 20px', borderRadius: 100, border: 'none', fontSize: 12, fontWeight: 600,
                cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' as any,
                background: activeCommunity === 'all' ? '#071629' : 'transparent',
                color: activeCommunity === 'all' ? '#fff' : '#666', transition: 'all 0.2s',
              }}>All</button>
              {communities.map(community => (
                <button key={community.id} onClick={() => setActiveCommunity(community.id)} style={{
                  padding: '8px 20px', borderRadius: 100, border: 'none', fontSize: 12, fontWeight: 600,
                  cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' as any,
                  background: activeCommunity === community.id ? '#3d9be9' : 'transparent',
                  color: activeCommunity === community.id ? '#fff' : '#666', transition: 'all 0.2s',
                }}>{community.name}</button>
              ))}
            </div>
            {communities.length === 0 && (
              <p style={{ fontSize: 12, color: '#666', textAlign: 'center', margin: '8px 0 0', fontStyle: 'italic' }}>
                Join a community below to start seeing posts!
              </p>
            )}
          </div>
                ...
                {openComments === post.id && (
                  <div style={{ padding: '8px 16px 14px', borderTop: '0.5px solid rgba(0,0,0,0.1)' }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                      ...
                      <div style={{ flex: 1, border: '1px solid rgba(0,0,0,0.15)', borderRadius: 32, padding: '8px 14px', display: 'flex', alignItems: 'center', gap: 8 }}>
                        <input
            ...
            <div style={{ padding: '14px 20px 0', display: 'flex', gap: 10, alignItems: 'center' }}>
              ...
              <div>
                ...
                <select
                  ...
                  style={{ fontSize: 12, border: '1px solid rgba(0,0,0,0.15)', borderRadius: 16, padding: '2px 10px', cursor: 'pointer', fontFamily: 'inherit', marginTop: 2, background: '#fff' }}
                >
                  {communities.map((c) => (
